...
while (my $line = <META>){
	...
	# Add a few more variables
        my $MQpar = $mqpardir . "/" . join ("_", $species,$sample,$BPP,"VAL_mqpar.xml");
        my $folderINIT = $species . '_' . "$sample/$species" . '_' . $sample . '_' . "$BPP/$species" . '_' . $sample . '_' . $BPP . '_INIT';
        my $folderINIT_SR = $species . '_' . "$sample/$species" . '_' . $sample . '_' . "$BPP/$species" . '_' . $sample . '_' . $BPP . '_INIT_SR';
        my $folderVAL = $species . '_' . "$sample/$species" . '_' . $sample . '_' . "$BPP/$species" . '_' . $sample . '_' . $BPP . '_VAL';
        my $folderVAL_SR = $species . '_' . "$sample/$species" . '_' . $sample . '_' . "$BPP/$species" . '_' . $sample . '_' . $BPP . '_VAL_SR';
...
#print "Species: $species\nSample: $sample\nRun: $BPP\nrawfile: $rawfile\nfastaDB: $fastaDB\nMQpar: $MQpar\nfolder: $folder\nexperiment: $experiment\n\n";
	...
	while (my $parline = <MQPAR>){
		...
		if ($parline =~ /PEPTIDEFILEPATH/){
			...
			$parline =~ s/(.*)FASTAFILEPATH(.*)/$1$indir\/$folderVAL\/FASTA\/$fastafile$2/; 
			print OUT $parline;
		}
