...
while (my $line = <META>){
	...
	# Add a few more variables
        my $SRpar = join ("_", $species,$sample,$BPP,$DBID,$MQrun,"srpar.xml");
        my $MQpar = join ("_", $species,$sample,$BPP,$DBID,$MQrun,"mqpar.xml");
        my $SRfolder = $species . '_' . "$sample/$species" . '_' . $sample . '_' . "$BPP/$species" . '_' . $sample . '_' . $BPP . '_' . $DBID . '_' . $MQrun . '_SR';
	my $MQfolder = $species . '_' . "$sample/$species" . '_' . $sample . '_' . "$BPP/$species" . '_' . $sample . '_' . $BPP . '_' . $DBID  . '_' . $MQrun;
...
#	my $cmd2 = "mkdir -p $indir/$SRfolder; mkdir -p $indir/$SRfolder/FASTA; cp $indir/$MQfolder/FASTA/*.fasta $indir/$SRfolder/FASTA"; # Need to check which fasta files should be in here...
	...
	# copy MQpar file to INIT or VAL folder
